"""
This doctest of for a UNIX platform.

>>> guesstimate(b'\x00f', ignore=False)
...
def guesstimate(llamas, ignore=True):
    translation_table = dict()
    codecs = codex()  # Instansiate the generator func

    for codec in codecs:
        try:
            if ignore is True:
                result = llamas.decode(codec, errors='ignore')
            else:
                result = llamas.decode(codec, errors='replace')

            translation_table[result] = translation_table.get(result, codec)

        except LookupError as e:  # platform dependancy for windows.
            print(e)
            continue

    translation_table = {char: codec for char, codec in translation_table.items()\
                         if char != '' and len(char) > 0 and char is not None}

    for char, codec in translation_table.items():
        print("{} : {}".format(char, codec), end='\n')
